...
def send_task_failure_email(alert):
    ...
    context = {
        ...
        "dashboard_url": (
            f"{settings.SITE_URL}/admin/document_processing/taskfailurealert/{alert.id}/change/"
...
def should_send_alert_email(task_name):
    ...
    enabled_tasks = config.get(
        "enabled_tasks", ["parse_document_task", "generate_audio_task"]
    )
...
def get_failure_summary():
    ...
    unresolved = all_failures.filter(
        status__in=["NEW", "ACKNOWLEDGED", "INVESTIGATING"]
    )

    by_task = (
        recent_7d.values("task_name").annotate(count=Count("id")).order_by("-count")
    )
