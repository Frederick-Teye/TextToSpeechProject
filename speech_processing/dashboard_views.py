...
from speech_processing.models import (
    Audio, AudioAccessLog, DocumentSharing, SiteSettings,
    AudioGenerationStatus, AudioLifetimeStatus, AudioAction
)
...
def analytics_data(request):
    ...
    voice_distribution = Audio.objects.filter(
        lifetime_status=AudioLifetimeStatus.ACTIVE
    ).values('voice').annotate(
        count=Count('id')
    ).order_by('-count')
    ...
    # Action distribution
    action_distribution = AudioAccessLog.objects.filter(
        timestamp__gte=start_date
    ...
    top_generators = Audio.objects.filter(
        created_at__gte=start_date
    ).values(
        'generated_by__email'
    ).annotate(
    ...
    top_active_users = AudioAccessLog.objects.filter(
        timestamp__gte=start_date
    ).values(
        'user__email'
    ).annotate(
    ...
    # Error rate by day
    error_trend = Audio.objects.filter(
        created_at__gte=start_date,
        status=AudioGenerationStatus.FAILED
    ).annotate(
        date=TruncDate('created_at')
